...
## make no change ref category
gdp_data$GDPWdiff_cat <- relevel(gdp_data$GDPWdiff_cat, ref = "no change")

## run unordered model
library(nnet)
mnl_model <- multinom(GDPWdiff_cat ~ REG + OIL, data = gdp_data)
summary(mnl_model)

z_vals <- summary(mnl_model)$coefficients / summary(mnl_model)$standard.errors
